...
def ensure_database(spark: SparkSession, db: str, location: Optional[str] = None) -> None:
    loc_sql = f" LOCATION '{location}'" if location else ""
    spark.sql(f"CREATE DATABASE IF NOT EXISTS {db}{loc_sql}")
    log.info(f"[DB] Ensured database/schema '{db}' exists")


def drop_database(spark: SparkSession, db: str, cascade: bool = True) -> None:
    spark.sql(f"DROP DATABASE IF EXISTS {db} {'CASCADE' if cascade else ''}")
    log.info(f"[DB] Dropped database/schema '{db}' (cascade={cascade})")
...
def _align_schema(source: DataFrame, target_schema: T.StructType) -> DataFrame:
    # Cast by name; add missing columns as nulls; drop extra columns
    src_cols = {c.name.lower(): c for c in source.schema}
    tgt_cols = [ (f.name, f.dataType, f.nullable) for f in target_schema ]
    selects = []
    for name, dtype, _nullable in tgt_cols:
        col = src_cols.get(name.lower())
        if col is None:
            selects.append(F.lit(None).cast(dtype).alias(name))
        else:
            selects.append(F.col(col.name).cast(dtype).alias(name))
    return source.select(*selects)


def aligned_to_existing_schema(spark: SparkSession, df: DataFrame, fqtn: str) -> DataFrame:
    if not table_exists(spark, fqtn):
        return df
    target_schema = spark.table(fqtn).schema
    return _align_schema(df, target_schema)


def purge_paths(*paths: Iterable[str] | str) -> None:
    for p in paths:
        ...
        if path.exists():
            ...
            log.info(f"[FS] Purged path {path}")